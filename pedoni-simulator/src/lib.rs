...
pub struct Simulator<M: PedestrianModel> {
    model: M,
    step: i32,
    pedestrian_spawn_buffer: Vec<Pedestrian>,
}

impl<M: PedestrianModel> Simulator<M> {
    // Prepare a new simulator with given options and scenario.
    pub fn new(model: M) -> Self {
        Simulator {
            model,
            step: 0,
            pedestrian_spawn_buffer: Vec::new(),
        }
    }
    ...
    pub fn spawn_pedestrian(&mut self, pedestrian: Pedestrian) {
        self.pedestrian_spawn_buffer.push(pedestrian);
    }
    ...
    pub fn tick(&mut self) -> StepMetrics {
        ...
        self.model.spawn_pedestrians(field, new_pedestrians);
        
        self.pedestrian_spawn_buffer
